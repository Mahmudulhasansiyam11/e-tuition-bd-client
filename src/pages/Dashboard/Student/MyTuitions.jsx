...
const MyTuitions = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (res.data.deletedCount > 0) {
      Swal.fire("Deleted!", "Tuition post deleted.", "success");
      refetch();
    }
  };
  ...
  const handleEdit = async (tuition) => {
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        ...
        <input id="classLevel" class="swal2-input" placeholder="Class" value="${tuition.classLevel}">
    ...
    const res = await axios.put(
      `${import.meta.env.VITE_API_URL}/tuitions/${tuition._id}`,
      formValues
    );

    if (res.data.modifiedCount > 0) {
      Swal.fire("Updated!", "Tuition post updated successfully.", "success");
      refetch();
    }
  };
